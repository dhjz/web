...
//				var freq = (rel_x * (total_freq) + 0) >> 0; // + 16 (min freq)
	...
	var ParagraphicModal = function ( app, custom_presets ) {
		...
		var x = new PKAudioFXModal ({
			...
			presets:[
					{name:'旧式电话机',val:'1,highpass,0,5800,5.8,1,lowpass,0,7060,5'}
			],
		...
		var TempoMetro = function ( app, modal ) {
			...
			function _make_ui ( q ) {
				...
				el_drawer.innerHTML = '<div class="pk_row">'+
					...
					'<label for="xxcjgs">强调拍</label></div>' + 

					'<div class="pk_row">'+
					'<a class="pk_modal_a_bottom" style="display:inline-block;float:none">播放节拍</a>'+
					'<a class="pk_modal_a_bottom" style="display:inline-block;float:none">播放曲目</a>'+
					'<a class="pk_modal_a_bottom" style="display:inline-block;float:none">同时播放</a>'+
					'</div>';
...
//						ctx.drawImage (tempCanvas, 0, 0, width, 100, -(raw_pixels.toFixed(1)/1), 0, width, 100);
		...
		var x = new PKAudioFXModal ({
			...
			body: '<div class="pk_tbs">' +
				'<a class="pk_tbsa pk_inact">节拍估算</a>' +
				'<a class="pk_tbsa">节拍测量</a>' +
