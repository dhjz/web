 ...
 *   SW 侧：只拦「同源、且在本 SW 作用域内」的 GET，重新包一层 Response 并附上 COOP/COEP。
 *   刷新后文档本身就由 SW 响应，于是 crossOriginIsolated === true（可实测
 *   window.crossOriginIsolated 与 typeof SharedArrayBuffer）。
 *
 * ⚠ 关键：**不要给跨域响应加头**（曾经踩过的坑，也是「页面能开、但 CDN 上的
 *   Vue / useUpload 一起挂掉」的根因）
 *   给跨域响应加 COEP: require-corp，等于宣称「这些跨域资源必须自带 CORP / 走 CORS」；
 *   对方是个不带这些头的普通 CDN 时，整条请求就被拦掉，而且报错发生在**别人的域名**下，
 *   光看页面代码根本查不出来。跨域子资源该不该放行，由浏览器按它自己的原始响应头判定，
 *   SW 不插手 —— 我们要隔离，靠的是**给自己文档加的 COOP** 在导航头层面触发。
 *
 * 前提（满足不了就只能改服务器/换地址）：
...
(function () {
  ...
  if (typeof window !== 'undefined') {
    ...
    var RELOAD_FLAG = 'coi-serviceworker-reloaded-v4';
  ...
  // 只处理「同源 + 在作用域内」的 GET。跨域、跨作用域一律放行、不碰。
  function isSameOriginInScope(request, scopeUrl) {
    if (request.method !== 'GET') return false;
    var url;
    try { url = new URL(request.url); } catch (e) { return false; }
    if (url.origin !== scopeUrl.origin) return false;            // 跨域 —— 绝不插手
    return url.href.indexOf(scopeUrl.href) === 0;                 // 作用域外 —— 也放行
  }
  ...
  self.addEventListener('fetch', function (event) {
    ...
    // ① 跨域 / 作用域外的请求原样放过。
    //    如果我们给跨域响应加盖 COEP: require-corp，浏览器就会拿「本页要求的跨域资源」
    //    去校验对方自己的 CORP/CORS；对方通常没有这些头，于是整条被拦 —— 页面本身没事
    //    （文档是我们自己加的 COOP 放行的），但 CDN 上的 Vue / useUpload / 字体全挂。
    var scopeUrl = new URL(self.registration.scope);
    if (!isSameOriginInScope(request, scopeUrl)) return;
    ...
    event.respondWith(
      fetch(request).then(function (response) {
        ...
        // ② 服务端自己配好的隔离头别覆盖（尤其别把明确的 require-corp 改掉）
